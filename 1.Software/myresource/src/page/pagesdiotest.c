...
static void Back_Button_Event(lv_event_t * e)
{
  ...
  {
    lv_timer_delete(pagesdiotest.timer);
    lv_obj_delete(pagesdiotest.view);
...
static void my_timer(lv_timer_t * timer)
{
  if(SD_DETECT == 0)
  {
    lv_obj_remove_state(pagesdiotest.sdio_write_file_btn, LV_STATE_DISABLED);
...
void Pagesdiotest_Init(void)
{
  ...
  pagesdiotest.back_btn = lv_button_create(pagesdiotest.view);
  ...
  pagesdiotest.sdio_write_file_btn = lv_button_create(pagesdiotest.sdio_view);
