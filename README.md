...
推荐使用的编辑环境为VSCode([下载地址链接](https://code.visualstudio.com/))，是一个运行于MacOS X、Windows和Linux之上的，针对于编写现代Web和云应用的跨平台源代码编辑器；vscode免费而且功能强大，对JavaScript和NodeJS的支持非常好，自带很多功能，例如代码格式化，代码智能提示补全、Emmet插件等。
...
该项目编译器为arm-none-eabi-gcc([下载地址链接](https://developer.arm.com/downloads/-/gnu-rm))
...
例子查看`hardware\src\lcd.c`。

如需修改屏幕参数，请在`hardware\inc\lcd.h`中修改。
...
请配合`FATFS`进行使用，例子查看`hardware\src\sdcard.c`。
...
主机还需配合`FATFS`，例子查看`hardware\src\usb_phy.c`、`hardware\src\usbh_msc_config.c`、`system\src\task.c`中的`USB_task`以及`third_party\fatfs\diskio.c`。
从机例子未使用，如需使用可查看`hardware\src\usbd_msc_config.c`，然后调用`usbd_msc_init`即可。
